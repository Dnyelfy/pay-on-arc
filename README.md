...
`?net=mainnet` or `?net=testnet` always wins.
A pay-link or subscribe-link without `?net` was made before mainnet existed,
so it opens on testnet — never on real money. New links always carry `net`.
The network this browser last switched to from the footer.
Otherwise mainnet, as long as its payments and subscriptions addresses are
